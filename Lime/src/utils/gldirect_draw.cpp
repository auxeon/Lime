...
#define glAssert(code) do{code; int l = __LINE__;\
   GLuint err = glGetError(); \
                ...
                { \
                  ...
                  std::cerr << "Message : " << (const char*)glGetString(err) << "("<<err<<")" << "\n";\
                  assert(false); \
              }\
}while(false)
...
public:
    ...
    /// GL_FRAGMENT_SHADER
    DShader(GLuint shaderType);

    /// compile shader from a text source file
    DShader(const std::string& source_path, GLuint shaderType);
...
public:
    ...
    Shader_prog(const DShader& vs, const DShader& fs);

    ~Shader_prog();

    /// set both vertex & fragment shaders (compiled)
    /// @return true if all went well
    int set_shaders(const DShader& vs, const DShader& fs);

    /// set any compiled shader
    /// @return true if all went well
    int set_shader(const DShader& vs);
...
DShader::DShader(GLuint shaderType)
{
...
DShader::DShader(const std::string& source_name, GLuint shaderType)
{
    _id   = glCreateShader(shaderType);
    _type = shaderType;
    GL_CHECK_ERRORS();
...
void DShader::add_define(const std::string& name, const std::string& value)
{
    _symbols[name] = value;
}
...
int DShader::load_file(const std::string& source_name) const
{
...
int DShader::load_source(const char* source) const
{
    std::string defines;
...
int DShader::check_status(const std::string& name) const
{
    int len;
...
DShader::operator GLuint() const{ return _id; }

// -----------------------------------------------------------------------------

DShader::~DShader(){ glAssert( glDeleteShader(_id) ); }
...
Shader_prog::Shader_prog(const DShader& vs, const DShader& fs):
    _id(glCreateProgram()),
    _gs_id(0)
{
...
int Shader_prog::set_shaders(const DShader& vs, const DShader& fs)
{
...
int Shader_prog::set_shader(const DShader& sh)
{
...
void init()
{
    ...
    DShader vertex(GL_VERTEX_SHADER  );
    DShader frag  (GL_FRAGMENT_SHADER);
