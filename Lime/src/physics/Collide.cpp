...
int Collide(Contact* contacts, RigidBody2DComponent* bodyA, RigidBody2DComponent* bodyB)
{
	// Setup
	Vec2 hA = 0.5f * Vec2(bodyA->size.x, bodyA->size.y);
	Vec2 hB = 0.5f * Vec2(bodyB->size.x, bodyB->size.y);

	Vec2 posA = Vec2(bodyA->position.x, bodyA->position.y);
	Vec2 posB = Vec2(bodyB->position.x, bodyB->position.y);

	Mat22 RotA(bodyA->rotation.z), RotB(bodyB->rotation.z);
